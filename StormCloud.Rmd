...
3.  **analysisTables** are tables derived from the analysisDF and are used to generate the analysis Result Tables.
...
#       alphabetically by event. These data.frames are the "analysisTables". 
# 2) the "analysisTables" are returned to ResultGenerationPhase, which passes 
...
#       set of plots;
#    4) the damage tables are named "analysisTables" and returned to the 
#       ResultGenerationPhase;
# 3) the ResultGenerationPhase function returns the "analysisTables" to the 
#    execution phase.
# 
# ------------------------------------------------------------------------------
# input variables
...
#
# LOCAL VARIABLES 
# analysisTables--the "analysisTables" are the product of the Data Assembly 
...
ResultGenerationPhase <- function() {
  ...
  analysisTables <- DataAssemblyPhase()
  resultTables   <- DataAnalysisPhase(analysisTables)
...
<pre>
Data Assembly Phase ============================================================
The Data Assembly Phase is responsible for creating the "analysisTables".
================================================================================
...
==============================================================================
 ...
 Get control from the Data Assembly Phase
    |      
    |                         return analysisTables to ResultGenerationPhase
    |                            ^
    V                            |
    +-->---+---------------------+
           | Data Assembly       |    analysisTables
           |    Phase            |<---<---<---<--<---<---<---<--<---<---+
           |                     |   analysisDF                         |
           |                     |<---<---<---<---<---<----<---<---+    ^
           |                     |  sourceDF                       ^    |
         ...
         Pass control to Read Storm File                      |    ^    |
           ...
           V   | Pass control to Assemble AnalysisTables      |    |    |
           |   V   |                                          |    ^    |
           --->|-->|-->--->+-------------+                    ^    |    ^
               |   V       | Read Storm  |                    |    ^    |
               |   |       |   File      |                    |    |    |
...
# 4) pass the "analysisDF" and control to the AssembleAnalysisTables function;
...
#    "analysisTables", 
# 6) return the "analysisTables" and control to AssembleAnalysisTables;
...
DataAssemblyPhase <- function()  {
  ...
  analysisTables <- AssembleAnalysisTables(analysisDF)
...
# 1     MOBILE    AL TORNADO          0       15    25.0          K       0
# 2    BALDWIN    AL TORNADO          0        0     2.5          K       0
# 
#   CROPDMGEXP
#   1           
#   2           
...
FinishAnalysisDataset <- function(stormDF) {
  ...
  colnames(analysisDF) <- eval(parse(text = DATA_ASSEMBLY_SELECT))
...
## 2) the "analysisDF" is passed to the MakeJurisAnalysisTables function,
...
#
# ==============================================================================
AssembleAnalysisTables <- function(analysisDF)
{
  jurisAnalysisTables <- MakeJurisAnalysisTables(analysisDF)
...
```{r MakeJurisAnalysisTables}
# MakeJurisAnalysisTables ======================================================
# The MakeJurisAnalysisTables function accepts the "analysisDF"
...
# ==============================================================================
MakeJurisAnalysisTables <- function(analysisDF) {
  ...
  for (i in 1:N_JURISDICTIONS) {
   ...
   jurisAnalysisDF <- analysisDF %>%
    ...
    jurisAnalysisTables <- 
      list.append(jurisAnalysisTables, jurisAnalysisDF)
  }
...
DataAnalysisPhase <- function(analysisTables)
{
  ...
  worstCasualtiesTables  <- MakeworstDamageTables(analysisTables, CASUALTIES_IDX)
  worstLossesTables    <- MakeworstDamageTables(analysisTables, LOSSES_IDX)

  worstCasualtiesDF  <- data.frame(worstCasualtiesTables)
  worstLossesDF    <- data.frame(worstLossesTables)
  resultTables <- list.append(resultTables, worstCasualtiesDF)
  resultTables <- list.append(resultTables, worstLossesDF)

  resultTables <- list.append(resultTables, MakeBarImages(worstCasualtiesDF, CASUALTIES_IDX))
  resultTables <- list.append(resultTables, MakeBarImages(worstLossesDF, LOSSES_IDX))

  set <- c(head(worstCasualtiesDF$EVTYPE, n = 10), 
                head(worstLossesDF$EVTYPE, n = 10))
  eventSet <- unique(set)
  resultTables <- list.append(resultTables, MakeCvLImages(analysisTables, eventSet))
...
# MakeworstDamageTables accepts the "analysisTables" and "analysisIndex" and 
...
#
# Input analysisTables are a set of data.frames contained in a list. Each 
...
# ==============================================================================
MakeworstDamageTables <- function(analysisTables, analysisIndex) {
  worstDamageTables = list()
  ...
  for (i in 1:N_JURISDICTIONS) {
    ...
    totalValue <- sum(table[, analysisIndex + 1])
...
#
# ==============================================================================
MakeBarImages <- function(worstDamageTables, index) {

  pltSet = list()

  for (i in 1:N_JURISDICTIONS) {
    worstDamageTables <- head(worstDamageTables, n=10)

    if (index == CASUALTIES_IDX) {
      table <- worstDamageTables %>% select(EVTYPE, CASUALTIES)
      ...
      table <- worstDamageTables %>% select(EVTYPE, LOSSES)
...
# MakeCvLImages ================================================================
# MakeCvLImages accepts the "analysisTables" and the "eventSet". This function
...
#
# Input analysisTables are a set of data.frames listing the analysis types, 
...
#
# ==============================================================================
MakeCvLImages <- function(analysisTables, eventSet) {
...
#
# ==============================================================================
resultTables <- ResultGenerationPhase()
