---
title: "StormCost: The Human and Economic Toll from U.S. Storms"
author: "Kevin Glass"
date: "`r Sys.Date()`"
output:
  html_document:
...
The StormCost code is complex and long; the base, uncommented code is around 400 lines and the testing code (shown in the Appendix 1) is another 200 lines. Some of the techniques used to produce the code have not been discussed in class as are some of the libraries.

## Setup Phase

<!-- 6789112345678921234567893123456789412345678951234567896123456789 -->

The Setup Phase is partitioned into three sections with each section .
...
GenerateStormCostTables <- function(url, dataFile)  {
  if (VERBOSE & GENERATE_STORM_COST_TABLES) {
    print("GenerateStormCostTables(): coordinate the generation of ")
...
GenerateSourceTableList <- function(sourceData) {
  ...
  for (i in COUNTY_SOURCE_INDEX : FEDERAL_SOURCE_INDEX) {
    ...
    if (DEBUG & GENERATE_SOURCE_TABLE_LIST) {
      print("GenerateSourceTableList: source table after filter select")
      print(head(sourceTable))
    }
...
GenerateStormCostAnalysis <- function(agencyTableList) {
  ...
  humanCost <- GenerateTop10HumanTable(agencyTableList)
  econCost  <- GenerateTop10EconomicTable(agencyTableList)
  
  ## Generate Human v. Economic Cost Plots
  cvcList <- GenerateCvC(humanCost)
