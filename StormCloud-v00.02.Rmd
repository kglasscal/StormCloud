...
MakeResultList <- function(analysisTables) {
  ...
  MakeBarImages(resultList[[1]], HUMAN_COST)
  MakeBarImages(resultList[[2]], ECON_COST)
...
MakeHvEImages <- function(analysisTables) {
  ...
  for (i in 1:length(DATA_INDICIES)) {
    plt <- ggplot(analysisTables[[i]], aes(x = HUMAN_COST, y = ECON_COST)) +
      ggtitle(JURISDICTION_LABELS[DATA_INDICIES[[i]]]) +
      xlab("Event") + ylab("Cost ($ millions)") + 
...
MakeBarImages <- function(top10Tables, type) {
  ...
  print(paste0("top10 type is ",class(top10Tables)))
  print(paste0("top10 type[1] is ",class(top10Tables[1])))
  print(paste0("top10[[1]] type is ",class(top10Tables[[1]])))
  print(top10Tables[[1]])
  print(top10Tables[[1]]$EVTYPE)
  ...
  for (i in 1:length(DATA_INDICIES)) {
      ...
      print(table)
      print(paste0("table type is ",class(table)))

      table$EVTYPE <- factor(table$EVTYPE, levels = table$EVTYPE)
      print(table)
      plt <- ggplot(top10Tables[[i]], aes(x = reorder(EVTYPE, -HUMAN_COST), y = HUMAN_COST )) + #x = EVTYPE, y = HUMAN_COST)) +
      ggtitle(JURISDICTION_LABELS[DATA_INDICIES[[i]]]) +
      xlab("Event Type") + ylab(AXIS_LABELS[[type]]) +
